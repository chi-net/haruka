...
pub(crate) async fn ledger_rows(
    ...
    for bill in bill::Entity::find().all(&state.db).await.map_err(err500)? {
        ...
        rows.push(LedgerRow {
            ...
            share_url: format!("/bills/{}/share", bill.id),
...
fn csv_field(value: &str) -> String {
    ...
    let protects_formula = trimmed
        .chars()
        .next()
        .is_some_and(|first| matches!(first, '=' | '+' | '-' | '@' | '\t' | '\r'));
    let value = if protects_formula {
        format!("'{value}")
    } else {
        value.to_string()
    };
...
async fn export_csv_response(
    ...
    response.headers_mut().insert(
        ...
        HeaderValue::from_str(&format!("attachment; filename=\"{filename}\"")).map_err(err500)?,
    );
    Ok(response)
}
...
async fn render_list(
    ...
    let html = BillsTemplate {
        ...
        export_action: if advanced_search {
            "/bills/search/export.csv".into()
        } else {
            "/bills/export.csv".into()
        },
        records,
