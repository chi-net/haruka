...
struct SubscriptionFormTemplate {
    ...
    accounts: Vec<AccountOption>,
    auto_debit: bool,
    auto_debit_account_id: i64,
    balance_check_days: i32,
}
...
struct ParsedSubscription {
    ...
    note: String,
    auto_debit_account_id: Option<i64>,
    balance_check_days: i32,
}
...
fn parse_form(form: SubscriptionFormData) -> HandlerResult<ParsedSubscription> {
    ...
    let balance_check_days = if form.balance_check_days.trim().is_empty() {
        3
    } else {
        form.balance_check_days
            .trim()
            .parse::<i32>()
            .map_err(|_| bad_request("余额检查提前天数格式不正确"))?
    };
    if !(0..=30).contains(&balance_check_days) {
        return Err(bad_request("余额检查只能提前 0 到 30 天"));
    }
    let auto_debit_account_id = if form.auto_debit {
        if form.auto_debit_account_id <= 0 {
            return Err(bad_request("启用自动扣款时必须选择扣款账户"));
        }
        Some(form.auto_debit_account_id)
    } else {
        None
    };
...
async fn validate_auto_debit_account(
    state: &AppState,
    ...
    let subscriptions = subscription::Entity::find()
        ...
        .map(|subscription| {
            ...
            SubscriptionRow {
                ...
                auto_debit: subscription.auto_debit_account_id.is_some(),
                auto_debit_label: subscription
                    .auto_debit_account_id
                    .and_then(|id| account_names.get(&id))
                    .map(|name| {
                        format!(
                            "自动扣款 · {name} · 提前 {} 天检查",
                            subscription.balance_check_days
                        )
                    })
                    .unwrap_or_default(),
            }
...
pub async fn new_form(
    ...
    let html = SubscriptionFormTemplate {
        ...
        auto_debit: false,
        auto_debit_account_id: 0,
        balance_check_days: 3,
    }
    .render()
    .map_err(err500)?;
    Ok(Html(html))
...
pub async fn create(
    ...
    subscription::ActiveModel {
        ...
        auto_debit_account_id: Set(parsed.auto_debit_account_id),
        balance_check_days: Set(parsed.balance_check_days),
...
pub async fn edit_form(
    ...
    let html = SubscriptionFormTemplate {
        ...
        accounts: account_options(&state, &dek).await?,
        auto_debit: subscription.auto_debit_account_id.is_some(),
        auto_debit_account_id: subscription.auto_debit_account_id.unwrap_or_default(),
        balance_check_days: subscription.balance_check_days,
    }
    .render()
    .map_err(err500)?;
    Ok(Html(html))
...
pub async fn update(
    ...
    active.auto_debit_account_id = Set(parsed.auto_debit_account_id);
    active.balance_check_days = Set(parsed.balance_check_days);
...
pub async fn create_expense(
    ...
    let subscription = subscription::Entity::find_by_id(id)
        ...
        .map_err(err500)?
        .ok_or((StatusCode::NOT_FOUND, "订阅不存在".into()))?;
    execute_expense(&state, &dek, subscription, form.account_id, false).await?;
    Ok(Redirect::to("/subscriptions"))
}

async fn execute_expense(
    state: &AppState,
    dek: &crypto::Dek,
    ...
    if automatic && account.currency != subscription.currency {
        return Err(bad_request("自动扣款账户与订阅货币不一致"));
    }
    let now = chrono::Utc::now().naive_utc();
    ...
    let note = if subscription_note.is_empty() {
        ...
        format!("{prefix}：{name} · {subscription_note}")
    };
...
pub async fn run_due(
    ...
    let _balance_guard = state.balance_writes.lock().await;
    let now = chrono::Utc::now().naive_utc();
    let due = subscription::Entity::find()
        .filter(subscription::Column::AutoDebitAccountId.is_not_null())
        .filter(subscription::Column::ExpiresAt.lte(now))
        .order_by_asc(subscription::Column::ExpiresAt)
        .all(&state.db)
        .await
        .map_err(err500)?;
    ...
    for item in due {
        ...
        match execute_expense(&state, &dek, item, account_id, true).await {
            Ok(()) => executed += 1,
            Err((_, error)) => failures.push(format!("{name}：{error}")),
        }
    }
    let message = if executed == 0 && failures.is_empty() {
        "没有待执行的自动扣款".into()
    } else if failures.is_empty() {
        format!("已完成 {executed} 笔自动扣款")
    } else {
        format!("完成 {executed} 笔，{} 笔未执行", failures.len())
    };
