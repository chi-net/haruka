...
pub struct AppState {
    ...
    market_fetches: Arc<tokio::sync::Mutex<()>>,
    sms_api_token_hash: Arc<String>,
    pub(crate) pending_sms: Arc<tokio::sync::Mutex<VecDeque<handlers::investments::PendingSms>>>,
...
impl AppState {
    ...
    pub fn any_session_dek(&self) -> Option<Dek> {
        self.sessions.read().unwrap().values().next().cloned()
    }

    pub async fn sms_token_matches(&self, token: &str) -> Result<bool, sea_orm::DbErr> {
        let configured_hash = entity::preference::Entity::find_by_id(1)
            .one(&self.db)
            .await?
            .map(|preferences| preferences.sms_api_token_hash)
            .unwrap_or_default();
        let expected_hash = if configured_hash.is_empty() {
            self.sms_api_token_hash.as_str()
        } else {
            configured_hash.as_str()
        };
        let supplied_hash = sms_api_token_hash(token);
        Ok(constant_time_eq(
...
pub(crate) fn sms_api_token_hash(token: &str) -> String {
...
async fn main() {
    ...
    let webauthn = WebauthnBuilder::new(&passkey_rp_id, &origin)
        ...
        .build()
        .expect("Passkey 配置无效");
    let sms_api_token = env::var("SMS_API_TOKEN")
        .ok()
        .filter(|token| !token.is_empty())
        .unwrap_or_else(|| {
            eprintln!(
                "警告：SMS_API_TOKEN 未配置，/api/sms 正在使用兼容令牌；对外部署前请设置随机长令牌"
            );
            "AppleToken".to_string()
        });
    let state = AppState {
        db,
        ...
        market_client: reqwest::Client::builder()
            .timeout(std::time::Duration::from_secs(20))
            .user_agent("haruka/0.1")
            .build()
            .expect("指数行情客户端初始化失败"),
        market_fetches: Arc::new(tokio::sync::Mutex::new(())),
        sms_api_token_hash: Arc::new(sms_api_token_hash(&sms_api_token)),
        pending_sms: Arc::new(tokio::sync::Mutex::new(VecDeque::new())),
    ...
    let protected = Router::new()
        ...
        .route("/bills/export.csv", get(handlers::bills::export_csv))
        .route(
            "/bills/search/export.csv",
            get(handlers::bills::export_advanced_csv),
        )
        .route("/bills/new", get(handlers::bills::new_form))
        .route(
            "/bills/{id}/share",
            get(handlers::shares::bill_share_form).post(handlers::shares::create),
        )
        .route(
            "/bills/{id}/edit",
        ...
        .route(
            "/investments/{id}/execute-manual",
            post(handlers::investments::execute_manual),
        )
        .route(
        ...
        .route(
            "/debt-requests",
            get(handlers::debt_requests::list).post(handlers::debt_requests::create),
        )
        .route("/debt-requests/new", get(handlers::debt_requests::new_form))
        .route(
            "/debt-requests/{id}/confirm",
        ...
        .route(
            "/debt-requests/{id}/delete",
            post(handlers::debt_requests::delete),
        )
        .route("/settings", get(handlers::settings::show))
        .route("/shares", get(handlers::shares::list))
        .route("/shares/{id}/delete", post(handlers::shares::delete))
        .route(
            "/settings/currency",
        ...
        .route(
            "/settings/owner-name",
            post(handlers::settings::update_owner_name),
        )
        .route(
            "/settings/sms-token",
            post(handlers::settings::update_sms_token),
        )
    ...
    let app = Router::new()
        ...
        .route(
            "/s/{token}",
            get(handlers::shares::open).post(handlers::shares::unlock),
        )
        .route(
            "/r/{token}",
            get(handlers::debt_requests::open).post(handlers::debt_requests::submit),
        )
        .route(
            "/r/{token}/account",
