...
pub async fn init() -> DatabaseConnection {
    ...
    db.execute(builder.build(create_preferences.if_not_exists()))
        .await
        .expect("建表失败");
    ensure_column(&db, "preferences", "owner_name", "TEXT NOT NULL DEFAULT ''").await;
    ensure_column(
        &db,
        "preferences",
        "sms_api_token_hash",
    ...
    db.execute(Statement::from_string(
        DbBackend::Sqlite,
        "INSERT OR IGNORE INTO preferences (id, default_currency, owner_name, sms_api_token_hash) VALUES (1, 'CNY', '', '')".to_string(),
    ))
    .await
    .expect("初始化默认货币失败");
    ...
    db.execute(builder.build(create_bill_shares.if_not_exists()))
        .await
        .expect("建表失败");
    let mut create_budgets = schema.create_table_from_entity(budget::Entity);
    db.execute(builder.build(create_budgets.if_not_exists()))
        .await
        .expect("建预算表失败");
    ...
    .await;
    ensure_column(&db, "investment_executions", "quote_date", "DATE").await;
    ensure_column(
        &db,
        "investment_executions",
        "index_close",
    ...
    .await
    .expect("创建定投执行唯一索引失败");
    let mut create_investment_sms_events =
        schema.create_table_from_entity(investment_sms_event::Entity);
    db.execute(builder.build(create_investment_sms_events.if_not_exists()))
        .await
        .expect("建定投短信事件表失败");
    ...
    let mut create_market_index_quotes =
        schema.create_table_from_entity(market_index_quote::Entity);
    db.execute(builder.build(create_market_index_quotes.if_not_exists()))
        .await
        .expect("建指数行情缓存表失败");
    ...
    db.execute(builder.build(create_debt_records.if_not_exists()))
        .await
        .expect("建表失败");
    let mut create_debt_requests = schema.create_table_from_entity(debt_request::Entity);
    ...
    .await;
    ensure_column(&db, "subscriptions", "auto_debit_account_id", "INTEGER").await;
    ensure_column(
        &db,
        "subscriptions",
        "balance_check_days",
