...
- `/dashboard` 对用户统一称为“仪表板”，集中快速记账、账户概览、转账、借还、统一流水及收支图表；日报按今日 24 小时、周/月/年报分别按近 7/30/365 个自然日聚合，Chart.js 使用平滑面积折线图。普通收支、转账与借还记录必须按时间合并显示在仪表板和 `/bills`，不再单列“资金动态”；普通收支汇总和图表仍不包含转账与借还。`/transfers`、`/debts` 的 GET 只重定向到仪表板。借贷对象由 `/debt-people` 独立管理并显示在顶部导航
- `/statistics` 是独立支出统计页，支持近 30 天、近 365 天和全部时间范围，至少展示分类排行和付款账户排行；统计仍须在 Rust 中解密后完成
