...
- 字段级加密：XChaCha20-Poly1305，每条随机 nonce，存 `base64(nonce||ct)` 于原 TEXT 列。加密字段：accounts.name/balance_offset/note、account_details.card_number/account_username/credit_limit、balance_adjustments.from_balance/to_balance、bills.amount/category/note、bill_shares.token/label、budgets.daily_amount/weekly_amount/monthly_amount、transfers.amount/to_amount/note、debt_people.name/note、debt_records.amount/note、categories.name、subscriptions.name/amount/category/note、preferences.owner_name、recurring_investments.name/amount/fee_rate_bps/sms_fund_name/note、investment_executions.base_amount/multiplier_bps、investment_sms_events.raw/detail、passkeys.name、installment_plans.annual_rate_bps/fee、installment_items.principal/interest/fee/total；`bill_shares.snapshot` 使用 URL 令牌与可选分享密码经 Argon2id 派生的独立密钥加密；账户类型、账单日、各记录 kind、账户外键、分类/账单的 is_food、happened_at、订阅周期/到期时间、定投策略/指数代码/均线周期/起始日/下一交易日/执行日期/行情审计字段/手续费账单外键、定投短信事件哈希/类型/状态/计划外键/转账外键/时间戳、公开指数行情缓存、市场休市日、分期期限/还款方式/还款日/计划状态、分期还款账户/自动流水外键、分享令牌哈希/盐/密码标记/有效期、Passkey 公钥凭据和时间戳为明文
...
- 默认货币存于单行 `preferences` 表，首次为 CNY；`preferences.sms_api_token_hash` 明文保存短信 API Token 的 SHA-256 校验值（不保存 Token 本身），空值回退到 `SMS_API_TOKEN` 环境变量或 `AppleToken`；账户原币存于 `accounts.currency`，订阅原币存于 `subscriptions.currency`。账单和借还金额以关联账户的原币计价，单账户余额始终显示原币；仪表板、账单汇总/金额搜索、借贷汇总和统计统一按当日最近可用汇率折算为默认货币，禁止直接相加不同货币金额
...
- 订阅服务存于 `subscriptions`，包含服务名、每次金额、支出分类、周期（day/week/month/quarter/year）、到期时间、备注、可选自动扣款账户和提前余额检查天数（0..=30）；自动扣款账户必须与订阅同币种。订阅页仍支持手动选择任意账户按汇率生成完全独立的普通 expense 账单；自动扣款只在已解锁浏览器访问仪表板后通过联网 POST 补执行，不代表连接银行或支付平台真实扣款。执行前必须校验普通账户余额或信用账户可用额度，失败时不得生成账单或顺延；成功后以到期时间和当前时间中较晚者为基准顺延一个周期。删除账户需清空相关自动扣款配置，删除订阅不得影响此前生成的账单
- `/investments` 是独立每日定投页。每个 `recurring_investments` 计划绑定一个非信用扣款账户和一个固定的 `investment` 基金账户，两端必须同币种。策略可选固定金额、聪明定投、每日手动金额或招商银行短信实扣；固定/聪明/手动模式按中国大陆交易日执行，手动模式由用户逐日填写实际金额。短信模式通过 `POST /api/sms` 接收 `{time,raw}` JSON，校验 `Authorization: Bearer <SMS_API_TOKEN>`（未配置时兼容 `AppleToken`），只解析受支持的招商银行成功/失败模板，并同时匹配扣款银行卡尾号与加密的基金名称；成功短信按实扣金额生成本金转账且不另记手续费，失败短信只生成加密审计提醒。短信日期按 `time` 的北京时间核对，所有模式以“计划 + 日期”唯一索引保证一天只扣一次。API 无 DEK 时原始短信只能暂存服务内存，解锁后处理，禁止明文落盘；服务重启可丢失未处理内存队列，处理后原文和结果加密保存。聪明定投支持沪深 300（000300）、中证沪港深 500（H30455）和恒生指数（HSI），均线周期可选 120/180/250/500 个指数交易日，默认 180。执行 T 日计划时取严格早于 T 的最近指数收盘价并用截至该日的 N 个收盘价计算均线；绝对偏离不超过 2% 时按基准金额 100%，高于均线 `(2,3]`/`(3,4]`/`(4,5]`/`(5,6]`/`(6,+∞)` 依次按 90%/80%/70%/60%/50%，低于均线同档依次按 110%/120%/130%/140%/150%。中证指数日线读取中证指数官网，恒生日线读取东方财富公开行情并持久缓存；更新失败时只可在缓存足够时明确警告后使用缓存，否则保留待执行并明确报错，禁止静默退回固定金额。每次执行须固化加密的基准金额/比例以及明文的指数代码、均线周期、行情日期、收盘点位和均线。本金生成普通 `transfers` 流水进入投资账户，不计入收入/支出；手续费率以加密整数基点存于 `fee_rate_bps`，实际手续费按“调整后本金 × 费率”四舍五入到分，额外从扣款账户扣除并生成分类为“投资手续费”的普通 expense 账单，计入支出统计。本金转账、手续费账单、执行记录和下一交易日必须在同一事务提交。自动转账和手续费账单在计划仍存在时禁止单独增删改，删除计划不得删除历史流水。中国交易日按北京时间纯日期判断：周一至周五且不在 `market_closed_days`；内置上交所 2025/2026 休市日，允许用户补充后续特殊休市日。DEK 仅在已解锁会话存在，因此锁定/重启期间只累计待执行日期，解锁访问定投页后由联网 AJAX 补执行，余额不足时保留待执行并明确报错。投资账户不记录基金代码、净值或份额；用户每月通过余额调整校准实际持仓总价值，差额增加或减少账户价值但不计入普通收入/支出
- 招商银行短信必须使用独立正则模板识别定投成功、定投失败、实时转至他行和快捷支付，并用 `time` 的北京时间核对短信年月日及时分。`preferences.owner_name` 加密保存本人姓名；实时转账收款人与本人姓名完全一致时只创建待确认候选，含“信用卡还款-本人姓名”的快捷支付同样只创建待确认候选。两者因短信缺少目标卡号，必须由用户选择同币种目标账户后才在事务中生成转账；信用卡还款目标只能是信用卡或信贷服务，普通扣款不得误判为定投或自动改余额
...
- 普通收入/支出账单可通过 `/bills/{id}/share` 创建不可变加密快照，公开路径固定为 `/s/{token}`。令牌使用 32 字节系统随机数并以 URL-safe Base64 编码，数据库只用 SHA-256 哈希检索，令牌明文仅以 DEK 加密保存供已解锁的分享管理页复制。快照使用“令牌 + 可选访问密码”经 Argon2id 派生的独立密钥加密，访问密码不得保存。有效期为 UTC 时间且每次公开访问都必须校验；无效、过期、撤销的链接统一显示不可用且不得泄露原因。公开页必须发送 `no-store`、`no-referrer`、`noindex` 和限制性 CSP，不能注册 Service Worker。原账单编辑或删除不改变已创建快照，用户可在 `/shares` 复制或撤销分享
- `/statistics` 是独立收支统计页，提供近 7/14/30/90/365 天快捷区间，也允许用户选择开始和结束日期（均包含）；至少展示收入、支出、结余、各自笔数与均值、支出中位数/最大单笔/日均/活跃消费日、食品支出占比、收支时间趋势、星期和单日支出分布、收入/支出的分类和账户排行及占比饼图，以及可跳转原账单的单笔支出 Top 20；多币种排行按默认货币折算，同时显示异币种账单原币金额，统计仍须在 Rust 中解密后完成
- `/budgets` 提供日、周、月三个可独立启用的预算，预算金额以默认货币计价并加密保存；日预算按自然日、周预算按周一至周日、月预算按自然月统计普通 expense 账单，转账、借还和定投本金不占预算。多币种支出按当日最近可用汇率折算；预算页和仪表板显示已用比例、剩余或超支金额，达到 80% 后以高优先级样式提醒
...
- 仪表板显示未来 7 日订阅到期和分期还款提醒，并保留已到期/逾期项目提醒；自动扣款进入用户配置的提前检查期后若余额或可用额度不足，必须在仪表板顶部以高优先级醒目提示。所有 `credit_service` 账户必须在仪表板独立高对比度展示当前欠款、已用额度、剩余额度、总授信额和使用比例，不能只混在普通账户卡片中
